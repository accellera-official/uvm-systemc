...
#
# Adapted from the Accellera SystemC cmake/run-test.py by Philipp A. Hartmann.
#
###############################################################################

"""
Run a UVM-SystemC regression test, including "golden log" file comparison,
logic adopted from tests/scripts/verify-uvm.pl
"""
...
UVM_HEADER_FILE = Path(env["UVM_HEADER_FILE"])
...
def detect_arch() -> str:
    arch = env["SYSTEMC_ARCH"]  # set by cmake from UVM_SYSTEMC_TARGET_ARCH
...
def strip_log(log_file: Path) -> Path:
    ...
    Mirrors the logic of the legacy verify-uvm.pl Perl script:
      1. strip_header  -- discard everything before "SystemC Simulation"
      2. strip_tail    -- discard everything from "End of SystemC Simulation"
      3. scl_strip     -- normalise variable paths and object addresses
      4. strip_outside_tags -- keep only "test started" ... "test ended"
      5. strip_uvm_header and strip_empty_lines
    """
    ...
    # 1. strip_header -- find "SystemC Simulation", drop everything before it
    sim_start = 0
    sim_end = len(log_lines)
    while sim_start < sim_end and not re_strip_head_pat.match(log_lines[sim_start]):
        sim_start += 1

    # 2. strip_tail -- find "End of SystemC Simulation", drop it and everything after
    ...
    # 3. scl_strip -- translation of verify-uvm.pl scl_strip substitutions
    systemc_home = env.get("SYSTEMC_HOME", "")
    re_strip_trace_timescale = re.compile(r"^Info: \(I702\) default timescale unit used for tracing:")
    re_strip_file_line = re.compile(r"^(In file: ).*$")
    re_strip_object_address = re.compile(r"@[0-9]+")
    re_strip_uvm_message = re.compile(r"(UVM_(?:INFO|WARNING|ERROR|FATAL))\s+.*\s+@\s+")
    normalized_lines = []
    for line in log_lines:
        if re_strip_trace_timescale.match(line):
            continue
        if systemc_home:
            line = line.replace(systemc_home, "$SYSTEMC_HOME")
        line = re_strip_file_line.sub(r"\1<removed by verify-uvm.pl>", line)
        line = re_strip_object_address.sub(r"@<removed by verify-uvm.pl>", line, count=1)
        line = re_strip_uvm_message.sub(r"\1 <removed by verify-uvm.pl> @ ", line, count=1)
        normalized_lines.append(f"{line.rstrip()}\n")
    log_lines = normalized_lines

    # 4. strip_outside_tags -- keep lines from "test started" to "test ended" (inclusive)
    tag_start = re.compile(r"test started")
    tag_end = re.compile(r"test ended")
    start_idx = next((i for i, line in enumerate(log_lines) if tag_start.search(line)), None)
    end_idx = next((i for i, line in enumerate(log_lines) if tag_end.search(line)), None)
    if start_idx is not None and end_idx is not None and end_idx >= start_idx:
        log_lines = log_lines[start_idx:end_idx + 1]
    elif start_idx is not None:
        log_lines = log_lines[start_idx:]
    elif end_idx is not None:
        log_lines = log_lines[:end_idx + 1]

    # 5. strip_uvm_header and strip_empty_lines
    uvm_header_lines = {line.rstrip() for line in UVM_HEADER_FILE.read_text().splitlines()}
    log_lines = [line for line in log_lines if line.rstrip() not in uvm_header_lines]
...
def compare_logfile() -> None:
    ...
    # Process the golden log like the legacy Perl flow: strip outside the
    # optional test tags and remove empty lines.
    tag_start = re.compile(r"test started")
    tag_end = re.compile(r"test ended")
    start_idx = next((i for i, line in enumerate(golden_lines) if tag_start.search(line)), None)
    end_idx = next((i for i, line in enumerate(golden_lines) if tag_end.search(line)), None)
    if start_idx is not None and end_idx is not None and end_idx >= start_idx:
        golden_lines = golden_lines[start_idx:end_idx + 1]
    elif start_idx is not None:
        golden_lines = golden_lines[start_idx:]
    elif end_idx is not None:
        golden_lines = golden_lines[:end_idx + 1]
    golden_lines = [f"{line.rstrip()}\n" for line in golden_lines if line.strip()]
